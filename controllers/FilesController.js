...
class FilesController {
   ...
   * If no file document is linked to the user and the ID passed as parameter,
   * return an error Not found with a status code 404
   * If the file document (linked to the user) is not public and not linked to
   * the user requesting it, return an error Not found with a status code 404
   * Otherwise:
   * If the type is folder, return an error A folder doesn't have content with a
   * status code 400
   * If the file is not locally present (on disk), return an error Not found with a
   * status code 404
  ...
  static async getFile(request, response) {
    ...
    if (!basicUtils.isValidId(id)) return response.status(404).send({ error: 'Not found' });
    ...
    if (!file.isPublic && !isOwner) return response.status(404).send({ error: 'Not found' });

    if (file.type === 'folder') { return response.status(400).send({ error: "A folder doesn't have content" }); }

    if (!file.localPath) return response.status(404).send({ error: 'Not found' });

    const absoluteFilePath = `${file.localPath}`;

    try {
      const mimeType = mime.lookup(file.name);
      response.setHeader('Content-Type', mimeType);
      return response.status(200).sendFile(absoluteFilePath);
    } catch (err) {
