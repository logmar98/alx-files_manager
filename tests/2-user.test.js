...
describe('testing User Endpoints', () => {
  ...
  describe('POST /users', () => {
    ...
    it('fails to create user because it already exists', async () => {
      await request(app).post('/users').send(user); // Create first user
  ...
  describe('GET /connect', () => {
    ...
    it('returns a token if user is found for credentials', async () => {
      ...
      const response = await request(app).get('/connect').set('Authorization', credentials).send();
  ...
  describe('GET /disconnect', () => {
    ...
    it('should sign-out the user based on the token', async () => {
      const response = await request(app).get('/disconnect').set('X-Token', token).send();
